        ...
        <div class="flex flex-wrap">
            <div class="w-full lg:w-1/2 my-6 pr-0 lg:pr-2">
                ...
                <div class="leading-loose">
                    <form class="p-10 bg-white rounded shadow-xl" method="POST" action="{{ route('course.store') }}"
                        ...
                        {{-- Teacher --}}
                        <div class="mb-4">
                            <label class="block text-sm text-gray-600" for="description">Teacher</label>
                            <textarea name="teacher" id="teacher" class="w-full px-5 py-2 bg-gray-200 rounded" placeholder="Course Teacher">{{ old('teacher') }}</textarea>
                            @error('teacher')
                                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        {{-- Start --}}
                        <div class="mb-4">
                            <label class="block text-sm text-gray-600" for="start_on">Start on</label>
                            <input type="date" name="start_on" id="start_on" value="{{ old('start_on') }}"
                        ...
                        {{-- End --}}
                        <div class="mb-4">
                            <label class="block text-sm text-gray-600" for="ends_on">End on</label>
                            <input type="date" name="ends_on" id="ends_on" value="{{ old('ends_on') }}"
                                class="w-full px-5 py-2 bg-gray-200 rounded" required>
                            @error('ends_on')
                                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="kuota" id="kuota" value="{{ old('kuota') }}"
                                class="w-full px-5 py-2 bg-gray-200 rounded" required placeholder="Misal: 50">
                            @error('kuota')
                                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
