...
@section('content')
    ...
    <script type="module">
        ...
        const matakuliahId = window.location.pathname.split('/')[2];
        ...
        async function loadLectures(selectedId = null) {
            try {
                ...
                lectures.forEach(lecture => {
                    ...
                    if (lecture.id == selectedId) opt.selected = true;
        ...
        async function loadData() {
            try {
                const res = await apiClient.get(`matakuliah/${matakuliahId}`);
                const data = res.data.data;

                document.getElementById('kode').value = data.kode;
                document.getElementById('nama').value = data.nama;
                document.getElementById('semester').value = data.semester;
                document.querySelectorAll('input[name="sks"]').forEach(input => {
                    if (input.value == data.sks) input.checked = true;
                });
                await loadLectures(data.dosen_id);
                ...
                console.error('Gagal memuat data matakuliah:', err);
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await apiClient.put(`/matakuliah/${matakuliahId}`, formData);
                console.log({
                    res
                });
                Swal.fire({
                    title: "Sukses!",
                    text: `${res.data.matakuliah.nama} Berhasil di tambahkan`,
                    icon: "success",
                    timer: 2500
                });
