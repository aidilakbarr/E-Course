        ...
        <div class="flex flex-wrap">
            <div class="w-full lg:w-1/2 my-6 pr-0 lg:pr-2">
                ...
                <div class="leading-loose">
                    <form class="p-10 bg-white rounded shadow-xl" method="POST" action="{{ route('table.store') }}">
                        @csrf
                        <div class="">
                            <label class="block text-sm text-gray-600" for="name">Name</label>
                            <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="name" name="name"
                                value="{{ old('name') }}" type="text" required="" placeholder="Your Name"
                                aria-label="Name">
                            @error('name')
                        ...
                        <div class="mt-2">
                            ...
                            <input class="w-full px-5  py-4 text-gray-700 bg-gray-200 rounded" id="email" name="email"
                                value="{{ old('email') }}" type="text" required="" placeholder="Your Email"
                                aria-label="Email">
                            @error('email')
                                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                            @enderror
